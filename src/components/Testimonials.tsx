...
import { getWordFrequencies } from "@/utils/wordFrequency";
...
export default function Testimonials() {
  ...
  const [view, setView] = useState<"carousel" | "cloud">("carousel");
  ...
  const wordData = useMemo(() => {
    const texts = sortedTestimonials.map((t) =>
      isEn
        ? t.originalLang === "es"
          ? t.feedbackTranslation
          : t.feedback
        : t.originalLang === "en"
          ? t.feedbackTranslation
          : t.feedback
    );
    return getWordFrequencies(texts, isEn ? "en" : "es");
  }, [isEn]);
  ...
  // Auto-advance
  useEffect(() => {
    if (hovered || expanded || view === "cloud") return;
    const id = setInterval(next, AUTOPLAY_MS);
    return () => clearInterval(id);
  }, [hovered, expanded, view, next]);
      ...
      <div className="section-container">
        ...
        >
          ...
          <div className="flex items-center justify-between mt-3 gap-4 flex-wrap">
            ...
            {/* View toggle */}
            <div className="flex items-center gap-1 p-1 rounded-full border border-border-dark bg-surface/50">
              <button
                type="button"
                onClick={() => setView("carousel")}
                className={`font-mono text-xs px-3 py-1.5 rounded-full transition-all duration-200 cursor-pointer ${
                  view === "carousel"
              ...
              >
                {isEn ? "Word Cloud" : "Nube de palabras"}
              </button>
            </div>
          </div>
        </motion.div>

        {/* Content: Carousel or Word Cloud */}
